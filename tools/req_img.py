"""
source code: https://github.com/OpenDCAI/DataFlow-Agent/blob/main/dataflow_agent/toolkits/imtool/req_img.py
"""
import os
import base64
import re
...
async def generate_or_edit_and_save_image_async(
    ...
    *,
    ...
    """
    根据模型类型选择不同的API进行图像生成/编辑（仅支持 Gemini 模型）
    ...
    if not _is_gemini_model(model):
        raise ValueError(f"仅支持 Gemini 系列模型，不支持: {model}")
